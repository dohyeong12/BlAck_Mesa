...
public class Red : MonoBehaviour
{
    ...
    public GameObject Left_F;
    public GameObject LArrow_F;
    ...
    void Start()
    {
        ...
        LArrow_F.SetActive(false);
    }

    // Update is called once per frame
    void Update()
    {
        //X를 누르면 0.5초후 r의 값에 따라 빨간깃발이 올라감
        ...
        //플레이어의 선택 후 새롭게 초기화된 r의 값이 0이 될 경우 빨간깃발이 올라가고 텍스트가 보여짐
        if (Flag_F.instance.r == 0 && Standby == false)
        {
            Left_F.SetActive(true);
            LArrow_F.SetActive(true);
            transform.rotation = Quaternion.Euler(0, 0, -1);
        }

        //플레이어의 선택으로 빨간깃발이 내려가고 텍스트가 사라지고 Flag의 pass를 3으로 초기화
        ...
        if (Flag_F.instance.pass == 0 && Standby == false)
        {
            ...
            StartCoroutine(RedPass2());
            StartCoroutine(RedPass());
        }
        else if (Flag_F.instance.pass == 1 && Standby == false)
        {
            ...
            StartCoroutine(RedPass2());
            StartCoroutine(RedFail());
        }

        //시간이 0이하가 되면 빨간 깃발이 올라감감
        if (a == 1)
        {
            Standby = true;
            transform.rotation = Quaternion.Euler(0, 0, -1);
